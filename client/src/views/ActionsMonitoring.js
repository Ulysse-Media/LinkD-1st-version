                        ...
                        {user.user_position === "MED" && (
                            <TableContainer component={Paper} style={{ height: '450px', overflow: "scroll" }}>
                                <Table className={classes.table} aria-label="simple table">
                                    ...
                                    <TableBody>
                                        {speakerActions.filter((element, key) => element.status === "En attente de validation CDP et MED" || element.status === "En attente de validation MED" || element.status === "Finalisée" || element.status === "Validée par CDP et en attente de retour agence").map((row) => (
                                            <TableRow key={row.action_id}>
                                                <TableCell>{row.user_email.split("@").shift()}</TableCell>
                                                <TableCell>{row.action_type}</TableCell>
                                                <TableCell>{row.meeting_theme}</TableCell>
