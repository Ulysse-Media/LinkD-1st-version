...
// Retrieve Notification by DSM supervisor 
...
// Retrieve Notification by CDP supervisor 
...
// Retrieve Notification by MED supervisor 
export function getNotificationsByMEDsupervisor(MED_supervisor) {
  return async (dispatch) => {
    dispatch({ type: FETCH_NOTIFICATIONS_BY_MED_SUPERVISOR_REQUEST });
    try {
      const response = await NotificationsServices.fetchNotificationsByMEDsupervisor(MED_supervisor);
      dispatch({ type: FETCH_NOTIFICATIONS_BY_MED_SUPERVISOR_SUCCESS, payload: response.data });
    } catch (e) {
      dispatch({
        type: FETCH_NOTIFICATIONS_BY_MED_SUPERVISOR_FAILURE,
      })
    }
  };
}
